...
def _compute_stats(
  ...
  Returns:
    A pair ``(mean, var)``.
  """
#   mask = _compute_mask_on_input(x) if mask is None else mask

  # === Add dynamic mask computation here ===
  if mask is None:
    mask = x != 0  # Treat all non-zero entries as valid
...
def _compute_mask_on_input(x: Array) -> Array:
  """Computes a mask on the input.

  Args:
    x: The input.

  Returns:
    A mask of the same shape as x.
  """
  mask = jnp.zeros(x.shape, dtype=x.dtype)
  mask = jnp.where(jnp.nonzero(x), 1.0, mask)
  return mask
    ...
    *,
    ...
    if use_mask:
        ...
        self.mask = nnx.Variable(
            nnx.initializers.ones(key, feature_shape, param_dtype)
        )
    else:
        self.mask = None
